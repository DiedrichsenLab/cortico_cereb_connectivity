...
# preparing data 
def prep_tensor():
   prep.save_data_tensor(dataset = "Nishimoto",
                        atlas='SUIT3',
                        ses_id='ses-01',
                        type="CondHalf")
   prep.save_data_tensor(dataset = "Nishimoto",
...
def train_models():
      ...
      for a in [0, 2, 4, 6, 8, 10, 12]:
         config = connect_run.get_train_config(log_alpha = a, dataset = "Nishimoto", ses_id = "ses-01")
      ...
      # save the dataframe
      filepath = os.path.join(prep.conn_dir, config['dataset'], 'Nishimoto_sub_train_model_ses-01.tsv')
...
def eval_models():
   ...
   df_eval_voxel_list = []
   for a in [0, 2, 4, 6, 8, 10, 12]:
      config = connect_run.get_eval_config(log_alpha = a, dataset = "Nishimoto", train_id = 'ses-01',eval_id = "ses-02")
   ...
   # save the dataframe
   filepath = os.path.join(prep.conn_dir, config['dataset'], 'Nishimoto_sub_eval_model_ses-02.tsv')
   df.to_csv(filepath, index = False, sep='\t')
   return
...
def get_best_weights(log_alpha=8, method = "L2Regression"):
   ...
   config = connect_run.get_train_config(log_alpha = log_alpha, dataset = "Nishimoto", ses_id = "ses-01")
   # get dataset class 
