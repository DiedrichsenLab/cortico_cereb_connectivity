...
conn_dir = ut.base_dir + '/../Cerebellum/connectivity/'
...
def extract_group_data(dataset="MDTB", ses_id='ses-s1'):
    ...
    Data = fdata.get_dataset_class(ut.base_dir, dataset=dataset)
...
def save_data_tensor(dataset="WMFS",
    ...
    data_tensor, info, Data = fdata.get_dataset(ut.base_dir,
                                                dataset,
                                                atlas=atlas,
                                                sess=ses_id,
                                                type="CondHalf",
...
def convert_cortex_to_cifti(data, atlas_cereb, atlas_cortex, fcn=np.nanmean):
    ...
    """
    pa_cereb = atlas_cereb.get_parcel_axis(
    )  # will be used to get the names of the parcels
    ...
    try:  # only runs if you have done .get_parcel
        ...
        for label in atlas_cortex.label_list:
            ...
            for p in np.arange(1, atlas_cereb.n_labels + 1):
                ...
                data_region = fcn(data[vox, :], axis=0)
                for i in np.unique(label):
                    np.put_along_axis(
                        label_arr[p - 1, :], np.where(label == i)[0], data_region[i - 1], axis=0)
            surf_map.append(label_arr)

        cifti_img = atlas_cortex.data_to_cifti(
            surf_map, row_axis=pa_cereb.name)
